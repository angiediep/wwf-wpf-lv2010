...
namespace ModelReader
{
    public class XOMLReader
    {
                    ...
                    if (tagName !="")
                    {
                        switch (tr.Name)
                        {
                            ...
                            case "SequenceActivity":
                                ...
                                break;
                            case "ns0:Work":
                                act = new WorkItem(i, new BusCongViec().getMoTaByTenCV(tr.GetAttribute("x:Name")), -1);
                                flag = 1;
                                break;
                        }
                        if (flag != 1)
        ...
        private void swapWorkItem()
        {
            List<int> listindex = new List<int>();
            for (int i = 0; i < m_ListActivity.Count; i++)
            {
                if ((m_ListActivity[i].Type == "ParallelActivity") || (m_ListActivity[i].Type == "SequenceActivity"))
                    m_ListActivity[i].swapWorkItem();
            }
        }

        /// <summary>
        /// lấy số lượng workitem trong file XOML
        /// </summary>
        /// <returns></returns>
